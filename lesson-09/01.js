...
petShop.addEventListener("click", function () {
  const clickedButton = event.target.closest(".pet");
  if (clickedButton && cart.length < 3) {
    cart.push(clickedButton.id);
    updateCartDisplay();
  } else {
    messageBox.innerHTML = "Вы не можете добавить более 3 питомцев";
  }
});
